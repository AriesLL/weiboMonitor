...
#studyMonth=$2
#studyDate=$3
#enforce=$4 #flag to enforce overwrite
player=$i
...
url="https://m.weibo.cn/api/container/getIndex?containerid=231343_${weiboID}_6"
#url=`echo "\"$4\""`
#echo $url
if [  -f "${starRankData}/weibo/${player}/raw/${playerChineseName}_20${cur_year}_${cur_month}_${cur_date}" ]; then
	cp ${starRankData}/weibo/${player}/raw/${playerChineseName}_20${cur_year}_${cur_month}_${cur_date} ${starRankData}/weibo/${player}/raw/${playerChineseName}_20${cur_year}_${cur_month}_${cur_date}_`date +%s`
fi

if [ ! -d "${starRankData}/weibo/${player}/raw/" ]; then
	mkdir -p "${starRankData}/weibo/${player}/raw/";
fi

wget -U Chrome -q -  $url -O "${starRankData}/weibo/${player}/raw/${playerChineseName}_20${cur_year}_${cur_month}_${cur_date}"
